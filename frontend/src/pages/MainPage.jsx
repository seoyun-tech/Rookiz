...
import { fetchTrending, fetchKidsMovies, fetchLatestKidsMovies, getImageUrl, fetchEnglishKidsContent } from "../api/tmdb";
    ...
    <div className="min-h-screen bg-white flex flex-col items-center">
      ...
      <section className="w-full max-w-[1280px] px-4 md:px-10 pt-4 md:pt-6 pb-6 md:pb-10">
        <div className="relative w-full aspect-[16/9] md:aspect-[2/1] rounded-2xl md:rounded-3xl overflow-hidden shadow-sm mx-auto">
          <img src={getImageUrl(kidsMovies[0]?.backdrop_path, "original")} className="absolute inset-0 w-full h-full object-cover" alt={kidsMovies[0]?.title} />
          ...
          <div className="absolute bottom-6 left-6 md:bottom-12 md:left-12 max-w-[90%] md:max-w-[672px] flex flex-col gap-2 md:gap-4 text-white">
            <div className="bg-primary-400 w-fit px-2 py-1 md:px-3 md:py-1.5 rounded-full flex items-center gap-1.5 md:gap-2">
              ...
              <span className="text-xs md:text-sm font-bold text-primary-700 font-sans">인기 1위</span>
            </div>
            <h1 className="text-2xl md:text-4xl lg:text-6xl font-black md:leading-10 font-sans">{kidsMovies[0]?.title}</h1>
            <div className="text-sm md:text-lg font-medium text-white/80 leading-snug md:leading-7">
              <p className="line-clamp-2">{kidsMovies[0]?.overview}</p>
            </div>
            <div className="flex gap-2 md:gap-4 mt-1 md:mt-2">
              <button onClick={() => kidsMovies[0] && openDetail(kidsMovies[0].id)} className="bg-primary-500 text-gray-700 px-4 py-2 md:px-8 md:py-4 rounded-[48px] flex items-center gap-1.5 md:gap-2 shadow-lg hover:bg-primary-400 transition-all font-bold text-sm md:text-2xl">
                <FontAwesomeIcon icon={faPlay} className="size-3 md:size-6" />
                <span>보러가기</span>
              </button>

              <button onClick={() => kidsMovies[0] && openDetail(kidsMovies[0].id)} className="bg-white/20 backdrop-blur-sm border border-gray-50 text-gray-50 px-4 py-2 md:px-8 md:py-4 rounded-[48px] flex items-center gap-1.5 md:gap-2 shadow-lg hover:bg-white/30 transition-all font-bold text-sm md:text-2xl">
      ...
      <main className="w-full max-w-[1280px] flex flex-col gap-6 md:gap-10">
        ...
        <div className="flex flex-col gap-8 md:gap-10 pb-20">
          <ContentRow title="글로벌 루키즈! 영어로 배워요" className="px-4 md:px-10">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-10">
              {englishContent
                ...
                .map((item) => (
                  <div key={item.id} className="aspect-[3/4] md:h-[360px] rounded-2xl md:rounded-[50px] overflow-hidden relative group cursor-pointer shadow-sm" onClick={() => openDetail(item.id, "tv")}>
                    <img src={getImageUrl(item.poster_path)} alt={item.name || item.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />

                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />

                    <div className="absolute bottom-4 left-4 right-3 md:bottom-6 md:left-6 md:right-4 text-white text-sm md:text-[21px] font-black leading-snug line-clamp-2">{item.name || item.title}</div>

                    <div className="absolute top-2 right-2 md:top-4 md:right-4 bg-blue-500 text-white text-xs font-bold px-2 py-0.5 md:py-1 rounded-full">🇺🇸 ENG</div>
                  </div>
                ))}
            </div>
          </ContentRow>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 lg:grid-rows-2 gap-4 md:gap-10">
              {kidsMovies?.slice(0, 5).map((item, i) => (
                <div key={item.id} className={i === 0 ? "col-span-2 lg:row-span-2" : ""}>
                  <Card size={i === 0 ? "lg" : "sm"} title={item.title || item.name} image={getImageUrl(item.poster_path)} className={i === 0 ? "aspect-[16/9] lg:aspect-auto lg:h-full" : "aspect-square"} onClick={() => openDetail(item.id)}>
          ...
          <ContentRow title="인기 콘텐츠" className="px-4 md:px-10">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-10">
              {kidsMovies.slice(0, 4).map((movie) => (
                <div key={movie.id} className="aspect-[3/4] md:h-[360px] rounded-2xl md:rounded-[50px] overflow-hidden relative group cursor-pointer shadow-sm" onClick={() => openDetail(movie.id)}>
                  <img src={getImageUrl(movie.poster_path)} alt={movie.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />

                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />

                  <div className="absolute bottom-4 left-4 right-3 md:bottom-6 md:left-6 md:right-4 text-white text-sm md:text-[21px] font-black leading-snug line-clamp-2">{movie.title}</div>
          ...
          <div className="mx-4 md:mx-10 min-h-[120px] md:h-[160px] bg-blue-900 rounded-2xl md:rounded-4xl flex flex-col md:flex-row items-start md:items-center justify-between p-6 md:px-8 relative overflow-hidden shadow-lg gap-4">
            ...
            <div className="flex flex-col gap-2 md:gap-5 relative z-10 text-gray-50">
              ...
              <p className="text-sm md:text-xl font-normal opacity-90">더 많은 혜택이 팡팡! 프리미엄으로 더 많은 프로필을 등록하세요!</p>
            </div>

            <button className="bg-primary-500 text-primary-950 px-6 py-2.5 md:px-8 md:py-4 rounded-full md:rounded-[48px] font-black text-sm md:text-2xl shadow-lg relative z-10 hover:bg-primary-400 transition-all shrink-0">보러가기</button>
          </div>
        </div>
      </main>
